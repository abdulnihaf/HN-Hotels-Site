// Amazon Fresh adapter — same shape as amazon_now.js but `i=amazonfresh` filter targets the Amazon Fresh sub-store. Best guess on filter value — if iPhone test or scout log shows wrong tile, swap to `i=amazon-fresh` — server-rendered HTML through DataImpulse proxy.
...
async function fetchAmazonHtml(query, session) {
  ...
  const url = `https://www.amazon.in/s?k=${encodeURIComponent(query)}&i=amazonfresh`;
...
async function scoutAmazonFreshDirect({ session, ctx }) {
  const query = ctx.searchQuery;
  let html;
  try {
    html = await fetchAmazonHtml(query, session);
  } catch (err) {
    return errorQuote(`Amazon Fresh fetch failed: ${err.message}`, { source: 'AMAZON_FRESH' });
  }

  const products = parseProducts(html);
  if (!products.length) {
    return emptyQuote(`Amazon Fresh returned no parseable products for ${query}`, {
      source: 'AMAZON_FRESH',
  ...
  if (pick.best) {
    return {
      ...
      eta_minutes: 240,
      eta_label: 'Amazon Fresh',
      match_confidence: pick.best.confidence,
      match_notes: 'Amazon Fresh HTML via DataImpulse proxy',
      raw: {
        source: 'AMAZON_FRESH',
  ...
  return emptyQuote(
    candidates.length
      ? `Amazon Fresh had ${candidates.length} SKUs but none passed filters for ${query}`
      : `Amazon Fresh parsed no products for ${query}`,
    { source: 'AMAZON_FRESH', candidate_count: candidates.length, html_size: html.length }
  );
}

export const scoutAmazonFresh = scoutAmazonFreshDirect;
