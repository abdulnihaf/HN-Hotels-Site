...
(function () {
  ...
  function rateText(i){
    ...
    return +i.price_paise>0 ? ('₹'+rupees(i.price_paise)+'/'+unitText(i)) : 'rate pending';
  }
  function calcText(i){
    if(chickenLine(i)){
      var yielded=lineYieldedKg(i), delivered=lineDeliveredKg(i), daily=lineDailyRatePaise(i), cost=lineAmount(i), eff=lineEffectivePaise(i);
    ...
    var q=qtyNum(i&&i.qty), p=+((i&&i.price_paise)||0), amt=lineAmount(i);
    if(q>0 && p>0) return q+' '+unitText(i)+' × ₹'+rupees(p)+'/'+unitText(i)+' = ₹'+rupees(amt);
    if(q>0) return q+' '+unitText(i)+' × rate pending';
  ...
  function operationalLineHtml(i, opts){
    ...
    return '<div class="oline '+(lineNeedsBill(i)?'need':'')+'">'+
      ...
      '<div class="oline-meta"><span>'+esc(qtyText(i))+'</span><span>'+esc(rateText(i))+'</span></div>'+
