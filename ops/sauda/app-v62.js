...
(function () {
  ...
  function renderOrder(){
    ...
    Object.keys(groups).forEach(function(vk){
      ...
      lines.forEach(function(l){
        ...
        var pc = qn>0 && pr>0 ? (qn+' '+(l.unit||'unit')+' × ₹'+pr.toLocaleString('en-IN')+'/'+(l.unit||'unit')+' = ₹'+Math.round(qn*pr).toLocaleString('en-IN')) :
          (qn>0 ? (qn+' '+(l.unit||'unit')+' × rate pending') : 'enter quantity and rate');
        html+='<div class="line"><div class="lhead"><div class="ln">'+esc(l.item)+(l.brand&&l.brand!=='both'?' <span class="lb">'+esc(l.brand)+'</span>':'')+'</div>'+
          ...
          '<div class="placecalc '+(qn>0&&pr>0?'ok':'warn')+'">'+esc(pc)+(l.live?' · live rate':'')+'</div>'+
  ...
  function setMode(m){
    ...
    else if(m==='orders'){ hist.classList.remove('hide'); if(h1) h1.textContent="Purchase day"; loadHistory(); }
  ...
  function brandPass(b, wanted){
    var nb=normBrand(b);
    if(!wanted || wanted==='all') return true;
    if(wanted==='HE') return nb==='HE' || nb==='both';
    if(wanted==='NCH') return nb==='NCH' || nb==='both';
    return nb===wanted;
  }
  ...
  function itemTitle(i){ return i&&String(i.item||i.name||i.raw||i.sku||'Item') || 'Item'; }
  function canonicalSku(i){
    var sku=String(i&&i.sku||i&&i.item_code||'').trim();
    if(sku && sku!==itemTitle(i)) return sku;
    if(i&&i.raw&&i.raw!==i.item) return i.raw;
    return '';
  }
  function unitText(i){ return String(i&&i.unit||'unit').trim() || 'unit'; }
  function billUnitText(i){ return String(i&&i.bill_unit||i&&i.unit||'unit').trim() || 'unit'; }
  function qtyText(i){
    var q=[i&&i.qty!=null?i.qty:'', i&&i.unit?i.unit:''].filter(Boolean).join(' ').trim();
    return q || 'qty pending';
  }
  function billQtyText(i){
    var b=i&&i.bill_qty!=null&&String(i.bill_qty).trim()!=='' ? [i.bill_qty, billUnitText(i)].filter(Boolean).join(' ') : '';
    return b || qtyText(i);
  }
  function hasSeparateBillQty(i){
    return !!(i&&i.bill_qty!=null&&String(i.bill_qty).trim()!==''&&String(i.bill_qty)!==String(i.qty));
  }
  function rateText(i){
    if(chickenLine(i)){
      var daily=lineDailyRatePaise(i), eff=lineEffectivePaise(i);
      if(daily>0) return 'live ₹'+rupees(daily)+'/kg';
      if(eff>0) return 'effective ₹'+rupees(eff)+'/kg';
      return 'live rate pending';
    }
    return +i.price_paise>0 ? ('₹'+rupees(i.price_paise)+'/'+billUnitText(i)) : 'rate pending';
  }
  function calcText(i){
    if(chickenLine(i)){
      var yielded=lineYieldedKg(i), delivered=lineDeliveredKg(i), daily=lineDailyRatePaise(i), cost=lineAmount(i), eff=lineEffectivePaise(i);
      if(delivered>0 && daily>0) return delivered+' live kg × ₹'+rupees(daily)+'/kg = ₹'+rupees(cost)+(yielded>0&&eff>0?' · '+yielded+' yielded kg · effective ₹'+rupees(eff)+'/kg':'');
      return 'Anbar receive needed: yielded kg + live kg + daily live rate';
    }
    var q=lineBillingQty(i), p=+((i&&i.price_paise)||0), amt=lineAmount(i), u=billUnitText(i);
    var prefix=hasSeparateBillQty(i)?('ordered '+qtyText(i)+' · billed '):'';
    if(q>0 && p>0) return prefix+q+' '+u+' × ₹'+rupees(p)+'/'+u+' = ₹'+rupees(amt);
    if(q>0) return prefix+q+' '+u+' × rate pending';
    return 'quantity pending';
  }
  function statusChips(i, opts){
    opts=opts||{};
    var chips=[];
    if(lineNeedsBill(i)) chips.push({c:'warn',t:chickenLine(i)?'Anbar/rate needed':'price needed'});
    else if(lineAmount(i)>0) chips.push({c:'ok',t:'bill basis ready'});
    if(chickenLine(i)){
      if(lineYieldedKg(i)>0 && lineDeliveredKg(i)>0) chips.push({c:'ok',t:'received'});
      else chips.push({c:'warn',t:'receive split needed'});
    } else if(i && (i.received_at || i.received_qty)) chips.push({c:'ok',t:'received'});
    if(i&&i.flag) chips.push({c:'warn',t:i.flag});
    if(opts.pay) chips.push({c:opts.payTone||'mut',t:opts.pay});
    return chips.map(function(x){ return '<span class="truth-chip '+esc(x.c)+'">'+esc(x.t)+'</span>'; }).join('');
  }
  function operationalLineHtml(i, opts){
    opts=opts||{};
    var sku=canonicalSku(i), total=lineAmount(i);
    return '<div class="oline '+(lineNeedsBill(i)?'need':'')+'">'+
      '<div class="oline-top"><div class="oline-name"><b>'+esc(itemTitle(i))+'</b>'+
        (sku?'<small>'+esc(sku)+'</small>':'')+'</div>'+
        '<strong>'+moneyText(total)+'</strong></div>'+
      '<div class="oline-calc">'+esc(calcText(i))+'</div>'+
      '<div class="oline-meta"><span>'+esc(hasSeparateBillQty(i)?('ordered '+qtyText(i)) : qtyText(i))+'</span>'+
        (hasSeparateBillQty(i)?'<span>'+esc('billed '+billQtyText(i))+'</span>':'')+
        '<span>'+esc(rateText(i))+'</span></div>'+
      '<div class="truth-chips">'+statusChips(i, opts)+'</div>'+
      (i&&i.received_note?'<div class="oline-note">'+esc(i.received_note)+'</div>':'')+
    '</div>';
  }
  function receiptState(items){
    items=items||[];
    if(!items.length) return {c:'mut',t:'no lines'};
    if(items.some(chickenLine)){
      return items.some(function(i){ return lineYieldedKg(i)<=0 || lineDeliveredKg(i)<=0; }) ? {c:'warn',t:'Anbar receive pending'} : {c:'ok',t:'Anbar received'};
    }
    return items.some(function(i){ return i&&i.received_at; }) ? {c:'ok',t:'received'} : {c:'mut',t:'receive not recorded here'};
  }
  function anbarReceiveTarget(o, items, brand){
    items=items||[]; brand=brand||orderBrand(o||{},items);
    var vendor=String(o&&o.vendor_name||'').toLowerCase();
    if(/broiler|m\.?\s*n/.test(vendor) || items.some(chickenLine)){
      return {brand:'HE', kind:'chicken', label:'Receive chicken in Ambar'};
    }
    if(brand==='NCH' || brand==='both'){
      return {brand:'NCH', kind:'all', label:'Receive in Ambar'};
    }
    return null;
  }
  function anbarReceiveUrl(o, target){
    var d=encodeURIComponent((o&&o.for_date)||defaultPurchaseDateIST());
    return 'https://hnhotels.in/ops/anbar/receive/?brand='+encodeURIComponent(target.brand)+'&date='+d+'&kind='+encodeURIComponent(target.kind);
  }
  function anbarReceiveCtaHtml(o, items, brand){
    var target=anbarReceiveTarget(o,items,brand);
    if(!target) return '';
    var rec=receiptState(items||[]);
    var ready=rec.c==='ok';
    return '<a class="handoff '+(ready?'done':'need')+'" href="'+esc(anbarReceiveUrl(o,target))+'" target="_blank" rel="noopener">'+
      '<span>'+esc(target.label)+'</span><b>'+esc(ready?'received':'pending')+'</b></a>';
  }
  function orderTruthHtml(o, items, opts){
    opts=opts||{}; items=items||[];
    var brand=opts.brand || orderBrand(o||{},items);
    var rec=receiptState(items);
    var pay=opts.pay || (o&&o.status ? String(o.status).toLowerCase() : 'order trail');
    var bank=opts.bankText || '';
    return '<div class="truth">'+
      '<div class="truth-row"><span>'+esc(brandText(brand))+'</span><span>'+esc(o&&o.fulfilmentLabel||o&&o.fulfilment||'source')+'</span><span>'+esc(o&&o.payLabel||o&&o.pay_timing||'payment')+'</span></div>'+
      '<div class="truth-chips"><span class="truth-chip '+esc(rec.c)+'">'+esc(rec.t)+'</span><span class="truth-chip mut">'+esc(pay)+'</span>'+(bank?'<span class="truth-chip mut">'+esc(bank)+'</span>':'')+'</div>'+
      (opts.note?'<div class="truth-note">'+esc(opts.note)+'</div>':'')+
    '</div>';
  }
  ...
  function renderPay(){
    ...
    groupMeta.forEach(function(g){
      ...
      rows.forEach(function(m){
        ...
        var itemsTxt=items.map(function(i){ return operationalLineHtml(i,{pay:needsRate?'not payable yet':'payable',payTone:needsRate?'warn':'ok'}); }).join('');
        ...
        var anbarCta=anbarReceiveCtaHtml(o,items,m.brand);
        ...
        html+='<div class="basket paycard'+(open?' open':'')+'" data-paykey="'+esc(key)+'"><div class="bh"><span class="chev">›</span><span class="bn">'+esc(o.vendor_name)+'</span><span class="payamt">'+moneyText(o.pay_amount_paise)+'</span>'+
          ...
          '<div class="pb" data-needs-rate="'+(needsRate?'1':'0')+'">'+dayLine+vendorNote+manualHint+orderTruthHtml(o,items,{brand:m.brand,pay:payStatusLabel(m.status),bankText:bankText})+anbarCta+(isChickenReceipt&&!rateItems.length?chickenCost:'')+'<div class="its olist">'+(itemsTxt||'—')+'</div>'+rateBox+
  ...
  function renderBuy(){
    ...
    host.innerHTML=rows.map(function(it){
      ...
      var vendor=it.vendor_name||it.vendor||it.default_vendor_name||(best&&best.source)||'source pending';
      var pack=it.your_pack||it.pack_label||it.unit||'unit pending';
      var price=+it.your_price_paise>0 ? ('usual ₹'+rupees(it.your_price_paise)) : (+it.price_paise>0 ? ('usual ₹'+rupees(it.price_paise)) : 'price to confirm');
      var miss=(!it.your_pack&&!it.pack_label&&!it.unit?'unit missing · ':'')+((+it.your_price_paise>0||+it.price_paise>0)?'':'price missing');
      return '<div class="hpitem'+(inb?' in':'')+'">'+
        '<div class="tap" style="cursor:default">'+photo+'<div class="nm"><b>'+esc(it.label||k)+'</b>'+
          '<span class="mt">'+esc(vendor)+'</span>'+
          '<div class="meta"><span class="pk">'+esc(pack)+'</span><span class="uu">'+esc(price)+'</span></div>'+
          (miss?'<span class="vd warn">'+esc(miss)+'</span>':'<span class="vd ok">ready for order</span>')+'</div></div>'+
  ...
  function orderLineText(it){
    var raw=(it&&it.raw&&it.raw!==it.item)?'<div class="truth-note">from '+esc(it.raw)+'</div>':'';
    return operationalLineHtml(it)+raw;
  }
  ...
  function uniqueJoined(arr, fallback){
    var seen={}, out=[];
  ...
  function batchLabel(o){
    return '#'+(o&&o.id!=null?o.id:'—');
  }
  function vendorGroupKey(o){
    return String((o&&o.vendorKey)||'').trim() || String(o&&o.vendor_name||'vendor').trim().toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'') || 'vendor';
  }
  function groupPlacedOrders(placed){
    var map={}, groups=[];
    (placed||[]).forEach(function(o,idx){
      var k=vendorGroupKey(o);
      if(!map[k]){
        map[k]={
          key:'placed-vendor-'+k,
          vendorKey:o.vendorKey||'',
          vendor_name:o.vendor_name||'Vendor',
          for_date:o.for_date||'',
          fulfilment:o.fulfilment||'',
          fulfilmentLabel:o.fulfilmentLabel||o.fulfilment||'',
          pay_timing:o.pay_timing||'',
          payLabel:o.payLabel||o.pay_timing||'',
          status:o.status||'',
          expected_amount_paise:0,
          ordered_at:o.ordered_at||'',
          ordered_by:o.ordered_by||'',
          brand:histBrand(o,o.items||[]),
          items:[],
          batches:[],
          order_count:0,
          ids:[]
      ...
      var g=map[k], items=o.items||[];
      g.order_count++;
      if(o.id!=null) g.ids.push(o.id);
      g.expected_amount_paise += (+o.expected_amount_paise||0);
      g.batches.push({ id:o.id, status:o.status||'', amount_paise:+o.expected_amount_paise||0, item_count:items.length, ordered_at:o.ordered_at||'', brand:histBrand(o,items), summary:summaryLines(items,2) });
      items.forEach(function(it,lineIdx){
        var cp=Object.assign({}, it);
        cp._batch_id=o.id;
        cp._batch_index=idx;
        cp._batch_label=batchLabel(o);
        cp._line_idx=(it&&it.line_idx!=null)?it.line_idx:lineIdx;
        if(cp.order_id==null && o.id!=null) cp.order_id=o.id;
        g.items.push(cp);
      });
      g.brand=mergeBrand(g.brand, histBrand(o,items));
      g.fulfilmentLabel=uniqueJoined([g.fulfilmentLabel,o.fulfilmentLabel||o.fulfilment],'');
      g.payLabel=uniqueJoined([g.payLabel,o.payLabel||o.pay_timing],'');
      g.status=uniqueJoined([g.status,o.status], g.status);
    });
    groups.forEach(function(g){
      if(g.order_count>1) g.status=g.order_count+' saved batches';
      g.items_json='';
    });
    return groups.sort(function(a,b){ return String(a.vendor_name||'').localeCompare(String(b.vendor_name||'')); });
  }
  function mergeBrand(a,b){
    a=normBrand(a); b=normBrand(b);
    return a===b ? a : 'both';
  }
  ...
  function renderPoCard(o,rowKey,open){
    ...
    return '<div class="hist-card'+(open?' open':'')+'" data-hkey="'+esc(rowKey)+'"><div class="hhd"><span class="chev">›</span><b>'+esc(brandText(brand))+'</b>'+
      ...
      '<div class="hbody">'+orderTruthHtml(o,items,{brand:brand,pay:'purchase input'})+items.map(orderLineText).join('')+'</div></div>';
  }
  function renderPlacedCard(o,rowKey,open){
    ...
    var batchNote=batches.length>1
      ? '<div class="hist-note">One vendor-day purchase. Saved in '+batches.length+' batches because items were added at different times; Sauda keeps the batch trail below for audit.</div>'+
        '<div class="batch-list">'+batches.map(function(b){
          return '<div class="batch-row"><b>'+esc('#'+(b.id||'—'))+'</b><span>'+esc((b.summary||'batch')+(b.status?' · '+b.status:''))+'</span><strong>'+moneyText(b.amount_paise)+'</strong></div>';
        }).join('')+'</div>'
      : '';
    ...
    return '<div class="hist-card'+(open?' open':'')+'" data-hkey="'+esc(rowKey)+'"><div class="hhd"><span class="chev">›</span><b>'+esc(o.vendor_name||'')+'</b>'+(amt?'<span class="hamt">₹'+rupees(amt)+'</span>':'')+
      ...
      '<span class="tag p">'+esc(brandChip(brand))+'</span><small>'+esc(o.status||'')+'</small>'+(o.order_count>1?'<small>'+o.order_count+' batches</small>':'')+'</span>'+
      '<span class="mini">'+esc(summaryLines(items,3)||'Vendor order')+'</span></div>'+
      '<div class="hbody">'+note+batchNote+orderTruthHtml(o,items,{brand:brand,pay:o.status||'order placed'})+anbarReceiveCtaHtml(o,items,brand)+chicken+items.map(orderLineText).join('')+'</div></div>';
  }
  ...
  function renderHistory(){
    ...
    var po=HIST_CACHE.po||[], placed=HIST_CACHE.placed||[], placedGroups=groupPlacedOrders(placed), s=HIST_CACHE.summary||{}, fd=HIST_CACHE.for_date||S.hist.date||defaultPurchaseDateIST();
    ...
    placedGroups.forEach(function(o){ var items=o.items||[]; rows.push({type:'placed', key:o.key, o:o, items:items, brand:histBrand(o,items)}); });
    ...
    if(head) head.innerHTML='<span class="ttl">'+esc(L.t)+(L.sub?' · '+esc(L.sub):'')+'</span>'+
      '<span class="fresh">'+filtered.length+' card'+(filtered.length===1?'':'s')+' · '+(s.po_items||0)+' input item'+((s.po_items||0)===1?'':'s')+' · '+placedGroups.length+' vendor purchase'+(placedGroups.length===1?'':'s')+' · '+(s.placed_orders||0)+' saved batch'+((s.placed_orders||0)===1?'':'es')+'</span>';
    ...
    if(!filtered.length){ list.innerHTML='<div class="empty small">No order matches this search/filter for '+esc(L.t)+'.</div>'; return; }
      var html='<div class="hist-summary">'+
        ...
        '<div class="hist-box"><b>'+esc(String(placedGroups.length||0))+'</b><span>vendor-day purchases</span></div>'+
  ...
  function exportPurchaseDayPDF(){
    var po=HIST_CACHE.po||[], placed=HIST_CACHE.placed||[], fd=HIST_CACHE.for_date||S.hist.date||defaultPurchaseDateIST();
    if(!po.length && !placed.length){ toast('Load a purchase day first','err'); return; }
    var groups=groupPlacedOrders(placed);
    var brandFilter=S.hist.brand||'all';
    var visibleGroups=groups.filter(function(g){ return brandPass(histBrand(g,g.items||[]), brandFilter); });
    var visiblePo=po.filter(function(o){ return brandPass(histBrand(o,o.items||[]), brandFilter); });
    var L=fmtDayLabel(fd);
    var vendorTotal=visibleGroups.reduce(function(s,g){ return s+(+g.expected_amount_paise||0); },0);
    var batchTotal=visibleGroups.reduce(function(s,g){ return s+(g.order_count||1); },0);
    var lineTotal=visibleGroups.reduce(function(s,g){ return s+((g.items||[]).length||0); },0);
    var receivePending=visibleGroups.reduce(function(n,g){ return n+(receiptState(g.items||[]).c==='ok'?0:1); },0);
    function pdfQty(i){ return esc(qtyText(i)||'—'); }
    function pdfRate(i){ return esc(rateText(i)||'rate pending'); }
    function pdfBill(i){ var amt=lineAmount(i); return amt>0 ? ('₹'+rupees(amt)) : '—'; }
    function pdfReceive(i){
      var r=receiptSummary(i);
      if(r) return esc(r);
      var st=receiptState([i]);
      return esc(st.t||'receive pending');
    }
    function lineRows(g){
      return (g.items||[]).map(function(i){
        var warn=lineAmount(i)<=0 || (chickenLine(i) && (lineYieldedKg(i)<=0 || lineDeliveredKg(i)<=0));
        return '<tr'+(warn?' class="warn"':'')+'><td>'+esc(i._batch_label||'')+'</td><td>'+esc(itemTitle(i))+(i.received_note?'<div class="note">'+esc(i.received_note)+'</div>':'')+'</td><td>'+pdfQty(i)+'</td><td>'+pdfRate(i)+'</td><td>'+pdfBill(i)+'</td><td>'+pdfReceive(i)+'</td></tr>';
      }).join('');
    }
    function poRows(o){
      return (o.items||[]).map(function(i){ return '<tr><td>'+esc(brandChip(histBrand(o,o.items||[])))+'</td><td>'+esc(itemTitle(i))+'</td><td>'+pdfQty(i)+'</td><td colspan="3">'+esc(i.raw&&i.raw!==i.item?('from '+i.raw):'purchase input')+'</td></tr>'; }).join('');
    }
    var poHtml=visiblePo.length
      ? '<section><h2>Purchase Inputs <span>'+visiblePo.length+' source card'+(visiblePo.length===1?'':'s')+'</span></h2><table><tr><th>Brand</th><th>Item</th><th>Ordered</th><th colspan="3">Input note</th></tr>'+visiblePo.map(poRows).join('')+'</table></section>'
      : '';
    var vendorHtml=visibleGroups.map(function(g){
      var brand=histBrand(g,g.items||[]), target=anbarReceiveTarget(g,g.items||[],brand), rec=receiptState(g.items||[]);
      var anbar=target ? anbarReceiveUrl(g,target) : '';
      var batchHtml=(g.batches||[]).length>1 ? '<div class="batches">'+(g.batches||[]).map(function(b){ return '#'+esc(b.id||'—')+' '+moneyText(b.amount_paise)+' '+esc(b.summary||''); }).join(' · ')+'</div>' : '';
      return '<section><h2>'+esc(g.vendor_name)+' <span>'+esc(brandChip(brand))+' · '+(g.order_count||1)+' saved batch'+((g.order_count||1)===1?'':'es')+' · '+moneyText(g.expected_amount_paise)+'</span></h2>'+
        '<div class="meta"><b>'+esc(g.fulfilmentLabel||g.fulfilment||'fulfilment')+'</b> · <b>'+esc(g.payLabel||g.pay_timing||'payment')+'</b> · <b class="'+(rec.c==='ok'?'ok':'warnc')+'">'+esc(rec.t)+'</b>'+(anbar?' · Ambar: '+esc(anbar):'')+'</div>'+batchHtml+
        '<table><tr><th>Batch</th><th>Item</th><th>Ordered</th><th>Rate / basis</th><th>Bill</th><th>Received / Ambar</th></tr>'+lineRows(g)+'</table></section>';
    }).join('');
    var today=new Date().toLocaleString('en-IN');
    var html='<!doctype html><html><head><meta charset="utf-8"><title>Sauda Purchase Day</title>'+
      '<style>body{font-family:-apple-system,Helvetica,Arial,sans-serif;margin:20px;color:#151515}'+
      '@page{size:A4 landscape;margin:9mm}h1{font-size:20px;margin:0 0 3px}.sub{font-size:11px;color:#666;margin-bottom:9px}.printbar{position:sticky;top:0;background:#fff;border:1px solid #ddd;border-radius:8px;padding:8px;margin-bottom:10px;display:flex;gap:8px;align-items:center}.printbar button{padding:7px 12px;border:0;border-radius:7px;background:#581810;color:#fff;font-weight:800}.printbar span{font-size:12px;color:#555}.summary{display:grid;grid-template-columns:repeat(5,1fr);gap:6px;margin:8px 0 11px}.box{border:1px solid #ddd;border-radius:7px;padding:7px}.box b{display:block;font-size:17px}.box span{display:block;font-size:10px;color:#666;text-transform:uppercase;letter-spacing:.04em}section{break-inside:avoid;margin:0 0 12px}h2{font-size:13.5px;margin:12px 0 4px;color:#581810;border-bottom:2px solid #581810;padding-bottom:3px;break-after:avoid}h2 span{float:right;color:#777;font-weight:500;font-size:10.5px}.meta,.batches{font-size:10.5px;color:#444;margin:0 0 5px;line-height:1.35}.batches{color:#7a4b11}.ok{color:#16743b}.warnc{color:#a45f00}table{width:100%;border-collapse:collapse;font-size:10.2px}th{text-align:left;background:#f3efe8;padding:5px;border:1px solid #d6d0c7}td{padding:5px;border:1px solid #e2e2e2;vertical-align:top}.warn td{background:#fff1dd}.note{font-size:9.5px;color:#7a4b11;margin-top:2px}@media print{body{margin:0}.printbar{display:none}}</style></head><body>'+
      '<div class="printbar"><button onclick="window.print()">Print / Save PDF</button><span>A4 landscape · one vendor-day purchase, with batch trail and Ambar receive status.</span></div>'+
      '<h1>Sauda — Purchase Day · '+esc(L.t)+(L.sub?' · '+esc(L.sub):'')+'</h1><div class="sub">Generated '+esc(today)+' · selected brand: '+esc(brandFilter==='all'?'All':brandText(brandFilter))+' · receiving truth remains in Ambar.</div>'+
      '<div class="summary"><div class="box"><b>'+visibleGroups.length+'</b><span>vendors</span></div><div class="box"><b>'+batchTotal+'</b><span>saved batches</span></div><div class="box"><b>'+lineTotal+'</b><span>order lines</span></div><div class="box"><b>₹'+rupees(vendorTotal)+'</b><span>bill basis</span></div><div class="box"><b>'+receivePending+'</b><span>receive pending</span></div></div>'+
      poHtml+vendorHtml+'</body></html>';
  ...
  var histPdfBtn=document.getElementById('histPdf');
  if(histPdfBtn) histPdfBtn.addEventListener('click', exportPurchaseDayPDF);
  ...
  function lineBrief(lines, pendingRate){
    ...
    return lines.slice(0,5).map(function(i){
      if(chickenLine(i)){
        ...
        return [itemTitle(i), qtyText(i)].filter(Boolean).join(' ') + pcs + usable + formula + effective + (i.received_note?' · '+i.received_note:'');
      }
      var r=receiptSummary(i);
      var p=+i.price_paise>0 ? (' · '+calcText(i)) : (pendingRate&&!i.direct?' · receipt/bill pending':'');
      return [itemTitle(i), qtyText(i)].filter(Boolean).join(' ') + (r?' · '+r:'') + p;
