...
// ── Winner-Intelligence ranker (migration 0023 / ranker_configs) ──────────────
// The active ranker is published by the RTX winner_intel.py backtest. The worker
// uses its Stage-A LOSS GATE live (the deployable, walk-forward-proven core: it
// eliminated all 29 circuit-trap picks the old gap-ranker made, e.g. SETL on
// 2026-06-30). The full Stage-B upside ranker (opening drive + relvol + range)
// is proven on history but needs the 09:15-09:40 bar feed (intraday_bars is
// currently dead), so live ranking falls back to a preopen-structure tilt and the
// witness says so honestly. NONE of this is an order surface — picks_json + the
// execution gate are untouched.
const RANKER_GATE_DEFAULT = {
  min_liq: 10.0, preopen_circuit_gap: 6.0, preopen_circuit_liq: 60.0,
  near_band_gap: 18.0, sell_imbalance: -0.35, max_runup: 12.0,
};
...
function lossGateLive(c, gate) {
  const g = gate || RANKER_GATE_DEFAULT;
  const reasons = [];
  const liq = c.turnover_cr || 0, gap = c.gap_pct || 0, imb = c.preopen_imbalance;
  if (liq < g.min_liq) reasons.push('illiquid');
  if (Math.abs(gap) >= (g.near_band_gap || 18)) reasons.push('near_circuit_band');
  if (gap >= (g.preopen_circuit_gap || 6) && liq < (g.preopen_circuit_liq || 60)) reasons.push('preopen_circuit_risk');
  if (gap >= (g.max_runup || 12)) reasons.push('already_exhausted_runup');
  if (imb != null && imb < (g.sell_imbalance ?? -0.35)) reasons.push('preopen_sell_imbalance');
  return reasons;
}
...
async function scanGapUp(env, db, cfg, today) {
  ...
  // Winner-Intelligence loss gate (migration 0023). Applied LIVE on preopen+EOD.
  ...
  for (const g of gappers) {
    ...
    const tb = Number(g.total_buy_qty || 0), ts = Number(g.total_sell_qty || 0);
    const imbalance = (tb + ts) > 0 ? +((tb - ts) / (tb + ts)).toFixed(3) : null;
    all.push({
      symbol: g.symbol,
      ...
      preopen_imbalance: imbalance,
      catalyst,
      // preopen rank: liquidity-weighted, capped-gap, imbalance-tilted, catalyst bonus.
      // Caps gap influence at 5% so it does NOT chase circuit-prone big gaps (the old bug).
      rank_score: Math.min(g.iep_change_pct, 5) * (liq >= 100 ? 1.0 : liq >= 25 ? 0.9 : 0.8)
                  * (catalyst ? 1.15 : 1.0) * (imbalance != null ? (1 + Math.max(-0.3, Math.min(0.3, imbalance))) : 1.0),
    });
  }
  // Stage A: split survivors (rankable) from rejected (predictable losers / circuit traps).
  const candidates = [], rejected = [];
  const reasonHist = {};
  for (const c of all) {
    const reasons = lossGateLive(c, ranker.gate);
    if (reasons.length) { rejected.push({ ...c, reject_reasons: reasons }); for (const r of reasons) reasonHist[r] = (reasonHist[r] || 0) + 1; }
    else candidates.push(c);
  }
  candidates.sort((a, b) => b.rank_score - a.rank_score);
  rejected.sort((a, b) => b.gap_pct - a.gap_pct);
  return {
    candidates, rejected, scanned: preRows.length, gated: gappers.length, fresh: true,
    ranker_version: ranker.version,
    no_loser_gate: { scored: all.length, survivors: candidates.length, gated_out: rejected.length, reasons: reasonHist },
  };
}
...
async function runGapEngine(env, db, today, cfg, carryover) {
  ...
  // ── WINNER INTELLIGENCE (migration 0023) — the causal selection witness ──
  // Loss gate eliminated predictable losers / circuit traps (proven: 29→0 vs the
  // old gap ranker). The full upside ranker (opening drive+relvol+range, +29% winner
  // capture in walk-forward) is ARMED but needs the 09:15-09:40 bar feed; until then
  // live ranking uses the preopen-structure tilt. Intelligence only — never an order.
  const rejectedTop = (scan.rejected || []).slice(0, 8).map(r => ({
    symbol: r.symbol, gap_pct: r.gap_pct, turnover_cr: r.turnover_cr, reasons: r.reject_reasons }));
  let recentMissed = [];
  try {
    const rm = await db.prepare(
      `SELECT missed_winners_json FROM missed_winner_autopsy WHERE trade_date < ? ORDER BY trade_date DESC LIMIT 1`
    ).bind(today).first();
    const arr = rm?.missed_winners_json ? JSON.parse(rm.missed_winners_json) : [];
    recentMissed = arr.filter(m => m.reason_later_wrong).slice(0, 3)
      .map(m => ({ symbol: m.symbol, day_pct: m.day_pct, was_rejected_for: m.reject_reasons, lesson: m.rule_change }));
  } catch {}
  const winnerIntel = {
    ...
    why_this: recommendedSymbol
      ? `${recommendedSymbol}: best of ${scan.candidates?.length || 0} loss-gate survivors on preopen structure (liquidity + capped gap + buy-imbalance). It cleared every loser/circuit filter.`
      : 'No name cleared the loss gate today — honest flat observation, no paper or broker entry.',
    why_not_top_missed: recentMissed,
    source_state: 'live_preopen',
  };

  // Write the live selection witness (intelligence trail; never an order surface).
  try {
    const gateExpR = machinePlan[0]?.rr_ratio ?? null;
    await db.prepare(`
      INSERT OR REPLACE INTO daily_selection_witness
        (trade_date, decision, selected_symbol, ranked_candidates_json, rejected_json,
         no_loser_gate_json, expected_r, expected_upside_pct, why_this, why_not_top_missed_json,
         source_state, execution_authority, picks_broker_facing, ranker_version, composed_at, composed_by)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    `).bind(
      today, decision, recommendedSymbol, JSON.stringify(winnerIntel.ranked_candidates),
      JSON.stringify(rejectedTop), JSON.stringify(scan.no_loser_gate || {}),
      gateExpR, machinePlan[0]?.target_pct ?? null, winnerIntel.why_this,
      JSON.stringify(recentMissed), 'live_preopen',
      executionAuthorized ? 'broker_facing_picks_authorized' : 'intelligence_plan_only',
      picksJson.length ? 1 : 0, winnerIntel.ranker_version, Date.now(), 'wealth-verdict',
    ).run();
  } catch (e) { console.log('selection_witness(non-fatal):', e?.message || e); }
  ...
  `).bind(
    ...
    JSON.stringify({
      ...
      winner_intelligence: winnerIntel,
      risk_flags: [
        executionAuthorized
