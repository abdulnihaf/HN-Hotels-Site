...
// ─────────────────────────────────────────────────────────
export function scoreSymbolSimple(rows) {
...
// ─────────────────────────────────────────────────────────
// Pick N most-liquid symbols in the recent window.
// "Most-liquid" = highest average traded value over last 60 days.
// ─────────────────────────────────────────────────────────
export async function selectLiquidUniverse(db, { topN = 30, lookbackDays = 60 }) {
  const since = `date('now','-${lookbackDays} days')`;
  // We compute traded value as close × volume averaged
  const r = await db.prepare(`
    SELECT symbol,
      AVG(close_paise * volume) AS avg_traded_value,
      COUNT(*) AS days,
      AVG(volume) AS avg_volume
    FROM equity_eod
    WHERE exchange='NSE' AND trade_date >= ${since}
      AND close_paise > 0 AND volume > 0
    GROUP BY symbol
    HAVING days >= ${Math.floor(lookbackDays * 0.7)}
...
export async function runBacktest(db, config = {}) {
  ...
  try {
    ...
    const universe = cfg.universe && cfg.universe.length
      ? cfg.universe
      : await selectLiquidUniverse(db, { topN: cfg.topN, lookbackDays: 60 });
    ...
    for (const signalDate of signalDates) {
      ...
      for (const sym of universe) {
        ...
        const scored = scoreSymbolSimple(hist);
        if (!scored) continue;
