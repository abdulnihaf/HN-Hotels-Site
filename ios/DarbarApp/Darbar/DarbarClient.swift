...
actor DarbarClient {
    ...
    // MARK: hiring — manpower suppliers (flow #1)
    func suppliers(token: String) async throws -> [HiringSupplier] {
        let d = try await get("/api/hiring-darbar", ["action": "suppliers"], token)
        return (try decoder.decode(SuppliersResponse.self, from: d)).suppliers ?? []
    }
    @discardableResult
    func logSupplierCall(id: Int, outcome: String, note: String?, roles: [String], token: String) async throws -> HiringSupplier? {
        var body: [String: Any] = ["supplier_id": id, "outcome": outcome]
        if let note, !note.isEmpty { body["note"] = note }
        if !roles.isEmpty { body["roles_requested"] = roles }
        let d = try await send(path: "/api/hiring-darbar", query: ["action": "log_call"], method: "POST", body: body, token: token)
