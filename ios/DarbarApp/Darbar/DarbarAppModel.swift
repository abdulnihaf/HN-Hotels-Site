...
final class DarbarAppModel: ObservableObject {
    ...
    // hiring (flow #1 — manpower suppliers to call)
    @Published var suppliers: [HiringSupplier] = []
    @Published var supplierFilter = "all"      // all | new | active | done
    ...
    // suppliers — bucketed client-side so filter taps are instant (one fetch)
    var suppliersFiltered: [HiringSupplier] {
        switch supplierFilter {
        case "new":    return suppliers.filter { $0.status == "new" }
        case "active": return suppliers.filter { $0.status == "called" || $0.status == "responded" }
        case "done":   return suppliers.filter { $0.status == "sent_jd" }
        default:       return suppliers.filter { $0.status != "not_relevant" && $0.status != "dead" }
        }
    }
    var supplierUncalled: Int { suppliers.filter { $0.status == "new" && $0.hasPhone }.count }
    ...
    func loadSuppliers() async {
        guard let t = await ensureToken() else { return }
        loadingSuppliers = true; defer { loadingSuppliers = false }
        suppliers = (try? await DarbarClient.shared.suppliers(token: t)) ?? []
        suppliersLoaded = true
    }

    func logSupplierCall(id: Int, outcome: CallOutcome, note: String? = nil, roles: [String] = []) async {
        await run("Logged · \(outcome.label)", refresh: .suppliers) {
            try await DarbarClient.shared.logSupplierCall(id: id, outcome: outcome.rawValue, note: note, roles: roles, token: $0)
        }
    }
