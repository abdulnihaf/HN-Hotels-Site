...
@MainActor
final class WealthSession: ObservableObject {
    ...
    init() {
        ...
        if ProcessInfo.processInfo.environment["WEALTH_UNLOCK"] == "1" {
            unlocked = true
        }
    ...
    func tryBiometric() {
        ...
        ctx.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics,
                           localizedReason: "Unlock Wealth") { ok, _ in
    ...
    func submit(pin: String) {
        ...
        // LOCAL unlock: the app embeds the dashboard key, so the PIN only guards
        // the surface on-device (no /api/whoami on the wealth API). Compare to the
        // stored owner PIN (seeded to 0305 on first run).
...
struct ContentView: View {
    @StateObject private var session = WealthSession()
