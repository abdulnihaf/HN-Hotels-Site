...
struct SaudaView: View {
    ...
    private func usual(_ it: SaudaCompareItem) -> String {
        ...
        return "usual \(NaamFmt.rupee(Double(it.your_paise!)/100))\(pk)"
    }
    ...
    @ViewBuilder private func payGroup(_ title: String, _ orders: [SaudaOrder], _ col: Color) -> some View {
        if !orders.isEmpty {
            ...
            HStack {
                ...
                Spacer()
                Text("\(orders.count) · \(NaamFmt.rupee(Double(sub)/100))")
        ...
        card {
            VStack(alignment: .leading, spacing: 9) {
                HStack(spacing: 8) {
                    ...
                    brandChip(o.brand)
                    Spacer()
                    Text(NaamFmt.rupee(o.amountRupees))
                ...
                ForEach(o.lines.prefix(8)) { ln in
                    HStack {
                        ...
                        if ln.linePaise > 0 {
                            Text(NaamFmt.rupee(Double(ln.linePaise)/100)).font(.system(size: 12, weight: .semibold)).foregroundStyle(HK.textDim)
                        }
                    }
                }
        ...
        card {
            DisclosureGroup {
                VStack(alignment: .leading, spacing: 8) {
                    HStack(spacing: 14) {
                        meta("billed", NaamFmt.rupee(v.billedRupees))
                        meta("paid", NaamFmt.rupee(v.paidRupees))
                ...
                HStack(spacing: 8) {
                    ...
                    Spacer()
                    Text(v.isDue ? NaamFmt.rupee(v.outstandingRupees) : "clear")
    ...
    private func trailRow(_ t: SaudaTrailEntry) -> some View {
        ...
        return HStack(alignment: .top, spacing: 9) {
            ...
            Spacer()
            Text(NaamFmt.rupee(t.amountRupees)).font(.system(size: 13, weight: .semibold)).foregroundStyle(paid ? HK.ready : HK.textDim)
        }.padding(.vertical, 3)
    }
            ...
            HStack(spacing: 12) {
                ...
                VStack(alignment: .trailing, spacing: 2) {
                    if (it.unit_price_paise ?? 0) > 0 {
                        Text("\(NaamFmt.rupee(Double(it.unit_price_paise!)/100))/\(it.unit ?? "u")").font(.system(size: 13, weight: .heavy)).foregroundStyle(HK.text)
                    }
    ...
    private func hpVerdict(_ it: SaudaHpItem) -> String {
        ...
        switch it.verdict {
        case "cheaper": return "\(it.pct ?? 0)% cheaper than your \(NaamFmt.rupee(Double(it.your_unit_paise ?? 0)/100))/\(it.your_unit ?? "u")"
    ...
    private func compareRow(_ it: SaudaCompareItem) -> some View {
        card {
            HStack(spacing: 12) {
                ...
                Spacer()
                VStack(alignment: .trailing, spacing: 2) {
                    Text("you pay \(NaamFmt.rupee(Double(it.your_paise ?? 0)/100))").font(.system(size: 12, weight: .semibold)).foregroundStyle(HK.textDim)
    ...
    private func compareResult(_ it: SaudaCompareItem) -> String {
        ...
        if it.beats_baseline == true, let s = it.cheapest_source, (it.save_unit_paise ?? 0) > 0 {
            return "\(s.uppercased()) · save \(NaamFmt.rupee(Double(it.save_unit_paise!)/100))/\(it.unit ?? "u")"
        }
        ...
        card {
            HStack(spacing: 10) {
                ...
                if it.isLive {
                    ...
                    Text(NaamFmt.rupee(it.priceRupees)).font(.system(size: 14, weight: .heavy)).foregroundStyle(HK.text)
                } else {
