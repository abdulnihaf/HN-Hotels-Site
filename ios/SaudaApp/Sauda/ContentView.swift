...
@MainActor
final class SaudaSession: ObservableObject {
    ...
    init() {
        ...
        if ProcessInfo.processInfo.environment["SAUDA_UNLOCK"] == "1" {
            unlocked = true
        }
    ...
    func tryBiometric() {
        ...
        ctx.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics,
                           localizedReason: "Unlock Sauda") { ok, _ in
    ...
    func submit(pin: String) {
        ...
        Task {
            do {
                // Sauda auth = POST ?action=auth {pin} → shared Diwan token. A returned
                // token proves the PIN; store BOTH so the data layer sends x-darbar-token.
                let token = try await SaudaClient.shared.auth(pin: pin)
                KeychainStore.set(pin, for: account)
                KeychainStore.set(token, for: "sauda-token")
                await MainActor.run {
                    self.unlocked = true
                ...
                await MainActor.run {
                    self.error = (error as? SaudaError)?.errorDescription ?? error.localizedDescription
                    self.verifying = false
                }
            }
        }
...
struct ContentView: View {
    @StateObject private var session = SaudaSession()
...
private struct UnlockView: View {
    @ObservedObject var session: SaudaSession
