...
export async function onRequest({ request, env }) {
  ...
  if (action === 'settle-context' && method === 'GET') {
    ...
    const emp = await db.prepare(`SELECT id, COALESCE(known_as,name) AS name, brand_label, pay_type, monthly_salary, daily_rate, phone, COALESCE(pay_lane,'monthly') AS pay_lane, notes, start_date, COALESCE(presence_confirmed,0) AS presence_confirmed, is_active, track_attendance FROM hr_employees WHERE id = ?`).bind(employeeId).first();
