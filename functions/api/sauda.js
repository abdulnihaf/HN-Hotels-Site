    ...
    for (const r of list) {
      ...
      trail.push({
        ...
        lines: (Array.isArray(items) ? items : []).map((i) => ({
          ...
          yielded_kg: i.yielded_kg || i.received_qty || '',
          delivered_kg: i.delivered_kg || i.bill_qty || i.live_qty || '',
          daily_rate_paise: Math.max(0, Math.round(+i.daily_rate_paise || 0)),
          cost_paise: Math.max(0, Math.round(+i.cost_paise || 0)),
          effective_price_paise: Math.max(0, Math.round(+i.effective_price_paise || +i.price_paise || 0)),
          received_pieces: i.received_pieces || '',
