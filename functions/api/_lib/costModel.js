...
export function adaptiveRiskPct(compositeScore, empiricalWinRate = null, empiricalAvgRR = null) {
  ...
  const b = empiricalAvgRR != null ? empiricalAvgRR : (
    ...
    compositeScore >= 60 ? 2.3 :  // Tighter R:R for marginal scores
    1
  );
  if (p <= 0 || b <= 0) return 0;
  const q = 1 - p;
  ...
  const ceiling =
    ...
    compositeScore >= 60 ? 0.01 :  // Half-size for early-engine band
    0.01;
