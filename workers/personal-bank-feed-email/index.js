/**
 * hn-personal-bank-feed — PERSONAL HDFC alert ingester (Nihaf SA 4005).
 *
 * Fully isolated from the company pipeline:
 *   - reads from D1 'hn-personal-finance' (separate database)
 *   - listens on personal-hdfc@alerts.hnhotels.in
 *   - matched by Gmail filter scoped to A/c 4005 / Card 8891
 *
 * Code identical to hn-bank-feed-email except instrument tag.
 *
 * Writes rows with source='hdfc', instrument='hdfc_sa_4005'.
...
// `(source, source_ref, direction, amount_paise, txn_at)` unique index.
// Emit IST midnight literal `YYYY-MM-DDT00:00:00+05:30` — no UTC roundtrip.
